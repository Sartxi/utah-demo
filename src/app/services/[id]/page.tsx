...
export default async function Page(props: { params: Promise<{ id: string, serviceId: string }> }) {
  ...
  const pageDetails: PageDetails | null = await getPageDetailsByName(name);
  ...
  const breadcrumbs: Crumb[] = [{ text: 'Services', page: 'services' }, { text: page.display_name ?? page.name }];
